...
|----|-------|-----------|------|----------|
| P0.1 | Remove `extern crate cortex_m` and unused nightly features from `src/solver/main.rs`; verify `cargo build --release` | 60 | - | 0 |
| P0.2 | Resolve `src/solver/actions.rs`: move types from `action_abstraction.rs` into it (or delete the file) | 60 | - | 2 |
| P0.3 | Fix `kmeans::fit_growbatch` early-stop: remove trailing `break;`; add a test that exercises 5+ iterations on synthetic data | 120 | - | 0 |
| P0.4 | Strip dead commented-out code: `gen_ochs`, `gen_emd(turn/river, ...)` in `gen_abstraction/main.rs`; non-sampling `cfr()` path in `cfr.rs` (or refactor into a clean `FullCFR` module for Phase 6) | 60 | - | 2 |
| P0.5 | Smoke test: `train()` 10k iters asserts BR finite and `Infoset.regrets` mutates; verify `rust_poker 0.1.5` builds on current stable; document swap target if not | 180 | P0.1, P0.3 | 0 |
...
| 7     | 5     | 1320               |
| 8     | 2     | 420                |
| **Total** | **42** | **~8400 min (~14 work-weeks at 10h/week)** |
