...
pub fn evaluate(hand: &hand::Hand) -> u16 {
    return LOOKUP_TABLE.get().expect("LOOKUP_TABLE not initialized").evaluate(hand);
}

/// Global lookup table. Backed by `OnceLock` for thread-safe lazy
/// initialization; the `lazy_static!` 1.5.0 originally used here
/// panics on concurrent first access.
pub static LOOKUP_TABLE: std::sync::OnceLock<Evaluator> = std::sync::OnceLock::new();

/// Force-init the lookup table. Call this once from main before any
/// thread spawns.
pub fn init_lookup_table() {
    let _ = LOOKUP_TABLE.get_or_init(Evaluator::init);
}
...
mod tests {
    ...
    fn bench_lookup(b: &mut Bencher) {
        let hand = hand::Hand::empty()
            + hand::CARDS.get().expect("CARDS")[0]
            + hand::CARDS.get().expect("CARDS")[1];
    ...
    fn test_2222() {
        let hand = hand::Hand::empty()
            + hand::CARDS.get().expect("CARDS")[0]
            + hand::CARDS.get().expect("CARDS")[1]
            + hand::CARDS.get().expect("CARDS")[2]
            + hand::CARDS.get().expect("CARDS")[3];
